...
<form method="POST" action="{{url('employee/update-leave-info/'.$employee_id)}}">
{{ csrf_field() }}
...
<div id="leaves">
    @if(count($leaves) > 0) @php $sl = 0; @endphp
        @foreach($leaves as $leave) @php $sl = $sl + 1; @endphp
            <div class="row pd-t-10" id="leaves_{{$sl}}">
                <div class="col-md-2 remove-space" id="type_{{$sl}}">
                    <select class="form-control" name="leave_type_id[]">
                        <option value="" label>Leave Type</option>
                        @foreach($leave_types as $leave_type)
                            <option value="{{$leave_type->id}}" @if($leave_type->id == $leave->leave_type_id) selected @endif>{{$leave_type->leave_name}}</option>
                        @endforeach
                    </select>
                </div>
            
                <div class="col-md-2 remove-space" id="allotment_{{$sl}}">
                    <input type="text" class="form-control" name="yearly_allotment[]" placeholder="Yearly Allotment" value="{{$leave->yearly_allotment}}"/>
                </div>
                ...
                <div class="col-md-2 remove-space" id="opening_balance_{{$sl}}">
                    <input type="text" class="form-control" name="opening_balance[]" placeholder="Opening Balance" value="{{$leave->opening_balance}}"/>
                </div>

                <div class="col-md-2 remove-space" id="carry_forward_{{$sl}}">
                    <select class="form-control" name="carry_forward[]">
                        <option value="" label>Carry Forward</option>
                        <option value="1" @if($leave->carry_forward == "1") selected @endif>Yes</option>
                        <option value="0" @if($leave->carry_forward == "0") selected @endif>No</option>
                    </select>
                </div>

                <div class="col-md-1 remove-space" id="max_carry_forward_{{$sl}}">
                    <input type="text" class="form-control" name="max_carry_forward[]" placeholder="Max C.F" value="{{$leave->max_carry_forward}}"/>
                </div>
                ...
                <div class="col-md-1" style="padding:0px;">
                    ...
                    <a href="javascript:void(0)" class="btn btn-danger" onclick="remove_leave_row('{{$sl}}')" style="width:15px;padding-left:7px;"><i class="fa fa-minus-circle"></i></a>
                </div>
            </div>
        @endforeach
    @else
...
<div class="row">
    <div class="col-md-4 mg-t-15 remove-space">
        ...
        <select class="form-control" name="leave_count_from">
            <option value="" label>Select Leave Count From</option>
...
<script>
    var leaves    = {{count($leaves)}};
    if(leaves == "" || leaves == 0) { leaves = 1;}
