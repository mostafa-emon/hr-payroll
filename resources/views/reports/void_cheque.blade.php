...
@section('content')
  ...
  <script>
    ...
    if(amount_in_word_format == 'crore_lakh_thousand' || amount_in_word_format == 'crore_lac_thousand') {
      ...
      document.getElementById("excelButton").href="/export-void-cheque?bank_id={{$bank_name}}&account_id={{$ac_number}}&book_no={{$cheque_book}}&supplier={{$supplier_name}}&from_date={{$from_date}}&to_date={{$to_date}}&total="+croreFormat; 
    }else{
      var millionFormat = '{{$total}}'.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
      document.getElementById("grandTotal").innerHTML = millionFormat;
      document.getElementById("excelButton").href="/export-void-cheque?bank_id={{$bank_name}}&account_id={{$ac_number}}&book_no={{$cheque_book}}&supplier={{$supplier_name}}&from_date={{$from_date}}&to_date={{$to_date}}&total="+millionFormat; 
    }
